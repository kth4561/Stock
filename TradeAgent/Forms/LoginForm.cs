...
namespace TradeAgent
{
    public partial class LoginForm : Form
    {
        ...
        private void OnLogin(string szCode, string szMsg)
        {
           ...
           {
               if (cbID.GetItemText(cbID.SelectedItem).Equals("sculove"))
                    SessionCtrl.DBPATH = TradeAgent.Properties.Resources.DATABASE_SCULOVE;
                else
                    SessionCtrl.DBPATH = TradeAgent.Properties.Resources.DATABASE_GWOOOK;
        ...
        {
            if (!cbID.GetItemText(cbID.SelectedItem).Equals("") && !this.tbPw.Text.Trim().Equals(""))
            {
                session.connect(cbServer.SelectedValue.ToString().Trim());
                session.login(cbID.GetItemText(cbID.SelectedItem), this.tbPw.Text, "");
            }
            else
            {
